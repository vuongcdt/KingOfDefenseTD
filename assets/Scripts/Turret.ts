...
export class Turret extends Component {
    ...
    @property({ type: Enum(GunType) })
    private towerType: GunType;
    @property
    private damage: number = 3;
    @property
    private speed: number = 1;

    private _muzzleDouble: Node;
    private _muzzleSingle: Node;
    ...
    start(): void {
        ...
        this._levelManager = this._store.getLevelManager().node;
        this._muzzleDouble = this.node.getChildByName("MuzzleDouble");
        this._muzzleSingle = this.node.getChildByName("MuzzleSingle");
    ...
    update(dt: number): void {
        ...
        if (this._countdown > this._reloadTime && this._listEnemy.length > 0 && this._levelTower > 0) {
            this._countdown = 0;
            this.attackEnemy();
        }
    }
    ...
    attackEnemy() {
        ...
        normalize.multiplyScalar(this._muzzleDouble.position.y);

        const gunBarrelNumber = this.gunBarrelNumbers[this._levelTower];
        const position = this._levelTower > 2
            ? this.node.getParent().position
            : this.node.getParent().position.subtract(normalize);
    ...
    shooting() {
        this._muzzleDouble.active = this.towerType == TowerType.GunTower && this._levelTower != 2;
        this._muzzleSingle.active = this.towerType == TowerType.GunTower && this._levelTower == 2;
        ...
        setTimeout(() => {
            this._muzzleDouble.active = false;
            this._muzzleSingle.active = false;
            if (this.towerType == TowerType.RocketTower) {
                this._avatar.spriteFrame = this.avatarSprites[this._levelTower];
            }
        }, 100);
